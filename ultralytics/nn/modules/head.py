...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "HCPRTDETRDecoder", "DQSARTDETRDecoder", "ObjectCountingModule", "DifficultyEstimator", "v10Detect", "YOLOEDetect", "YOLOESegment"
...
class ObjectCountingModule(nn.Module):
    """
    Object Counting Module - 目标计数模块

    预测图像中的目标数量，用于 DQSA 动态查询选择。
    ...
    Examples:
        >>> counter = ObjectCountingModule(in_channels_list=(512, 1024, 2048), hidden_dim=256)
        >>> features = [torch.randn(2, 512, 64, 64), torch.randn(2, 1024, 32, 32), torch.randn(2, 2048, 16, 16)]
        >>> count = counter(features)
        >>> print(count.shape)  # torch.Size([2, 1])

    References:
        - Class-Agnostic Counting (CVPR 2021)
        - Few-Shot Object Counting (ECCV 2020)
    """

    def __init__(self, in_channels_list=(512, 1024, 2048), hidden_dim=256):
        """Initialize Object Counting Module with multi-scale feature projection and counting head."""
        super().__init__()
    ...
    def _reset_parameters(self):
        """Initialize parameters using Kaiming initialization for conv and normal distribution for linear layers."""
    ...
    def forward(self, features):
        """
        Forward pass to predict object count.

        Args:
            features (list): Multi-scale features from backbone
                [torch.Tensor([B, C1, H1, W1]), torch.Tensor([B, C2, H2, W2]), ...]

        Returns:
            count (torch.Tensor): Predicted object count [B, 1]
        """
        # 1. 投影到统一维度并上采样到最大分辨率
...
class DifficultyEstimator(nn.Module):
    """
    Difficulty Estimator - 难度估计网络

    预测图像的检测难度，用于 DQSA 动态查询选择。
    ...
    Examples:
        >>> estimator = DifficultyEstimator(in_channels_list=(512, 1024, 2048), hidden_dim=256)
        >>> features = [torch.randn(2, 512, 64, 64), torch.randn(2, 1024, 32, 32), torch.randn(2, 2048, 16, 16)]
        >>> difficulty = estimator(features)
        >>> print(difficulty.shape)  # torch.Size([2, 1])

    References:
        - Dynamic Head (CVPR 2021): Scale-aware attention
        - DetectoRS (arXiv 2020): Recursive feature pyramid
    """

    def __init__(self, in_channels_list=(512, 1024, 2048), hidden_dim=256):
        """Initialize Difficulty Estimator with multi-scale feature projection and difficulty prediction head."""
        super().__init__()
    ...
    def forward(self, features):
        """
        Forward pass to predict detection difficulty.

        Args:
            features (list): Multi-scale features from backbone

        Returns:
            difficulty (torch.Tensor): Predicted difficulty score [B, 1] in range [0, 1]
        """
...
class DQSARTDETRDecoder(nn.Module):
    ...
    Attributes:
        ...
        object_counter (ObjectCountingModule): 目标计数模块
        difficulty_estimator (DifficultyEstimator): 难度估计模块
    ...
    References:
        ...
        - RT-DETR (arXiv 2023): Real-Time Detection Transformer
    """
    ...
    def __init__(
        ...
        """Initialize DQSA RT-DETR Decoder with dynamic query selection and sample awareness."""
        super().__init__()
    ...
    def _reset_parameters(self):
        """Initialize parameters for encoder and decoder heads."""
        bias_cls = bias_init_with_prob(0.01) / 80 * self.nc
        constant_(self.enc_score_head.bias, bias_cls)
        constant_(self.enc_bbox_head.layers[-1].weight, 0.0)
        constant_(self.enc_bbox_head.layers[-1].bias, 0.0)
    ...
    def compute_adaptive_nq(self, x_features):
        """
        Compute adaptive query number based on predicted object count and difficulty.

        Args:
            x_features (list): Original backbone features (before projection)

        Returns:
            nq_per_image (torch.Tensor): Query number per image [batch_size]
            pred_count (torch.Tensor): Predicted object count [batch_size, 1]
            pred_difficulty (torch.Tensor): Predicted difficulty [batch_size, 1]
        """
        # 1. 预测目标数量
    ...
    def _get_encoder_input(self, x: List[torch.Tensor]) -> Tuple[torch.Tensor, List[List[int]]]:
        """Process encoder input by projecting and flattening backbone features."""
        feats = []
        shapes = []
    ...
    def _generate_anchors(
        ...
        """Generate anchor boxes for given feature map shapes."""
        anchors = []
        for i, (h, w) in enumerate(shapes):
    ...
    def _get_decoder_input(
        ...
        """
        Generate decoder input with adaptive query selection.

        Args:
            feats: Encoder features
            shapes: Feature map shapes
            dn_embed: Denoising embeddings
            dn_bbox: Denoising bboxes
            adaptive_nq: Adaptive query numbers per image [batch_size]
        ...
        # 提取 top-k 特征
        top_k_features = features[batch_ind, topk_ind].view(bs, effective_nq, -1)
        top_k_anchors = anchors[:, topk_ind].view(bs, effective_nq, -1)
    ...
    def forward(self, x: List[torch.Tensor], batch: Optional[dict] = None) -> Union[Tuple, torch.Tensor]:
        """
        Forward pass with dynamic query selection.

        Args:
            x (list): Multi-scale features from backbone
            batch (dict, optional): Batch info for training

        Returns:
            Training: (dec_bboxes, dec_scores, enc_bboxes, enc_scores, dn_meta, dqsa_losses)
            Inference: detection results [nq, 4+nc]
        """
        from ultralytics.models.utils.ops import get_cdn_group
